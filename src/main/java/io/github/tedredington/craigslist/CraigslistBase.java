...
/** Parent of every category query class. Each subclass implements
 *  {@link Iterable Iterable&lt;Listing&gt;} with transparent auto-pagination;
 *  the query itself is the result.
 *
 *  <p>Subclasses are sealed — there is one for each Craigslist category
 *  (Community, Events, ForSale, Gigs, Housing, Jobs, Resumes, Services). */
public abstract sealed class CraigslistBase implements Iterable<Listing>
    ...
    /** The approximate total count Craigslist reports on the first results page.
     *  Returns 0 when there are no results. Memoized per query instance. */
    ...
    /** Fetches and parses the listing's detail page, returning a new {@link Listing}
     *  with body text and (when available) geotag populated. */
