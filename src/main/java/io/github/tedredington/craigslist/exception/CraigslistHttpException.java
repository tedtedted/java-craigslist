...
/** Thrown when Craigslist returned a non-2xx HTTP status after retries were exhausted.
 *  Carries the status code, the URI, a truncated response-body snippet for diagnostics,
...
public class CraigslistHttpException extends CraigslistTransportException {
    ...
    public CraigslistHttpException(
            int statusCode, URI uri, String responseBody, int attemptCount) {
        super(
                "HTTP " + statusCode + " from " + uri + " (attempts=" + attemptCount + ")",
                uri);
