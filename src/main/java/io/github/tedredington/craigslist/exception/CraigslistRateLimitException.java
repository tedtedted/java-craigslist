...
/** Thrown when Craigslist responded with HTTP 429 (Too Many Requests). When the response
 *  included a {@code Retry-After} header, its value is exposed via {@link #retryAfter()}
 *  so callers can wait the suggested amount before retrying. */
